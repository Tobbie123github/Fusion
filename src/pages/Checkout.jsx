...
import { useCartStore, useAuthStore } from "../context/store"; // ← single import
import { Divider, Spinner } from "../components/UI";
import toast from "react-hot-toast";

// ─── OUTSIDE COMPONENT ───────────────────────────────────────────────────────
const ep = (item) => (item.discount > 0 ? item.discount : item.price); // effective price
...
function OrderSummary({ items, shipping, total }) {
  return (
    <div
      style={{
        background: "var(--card)",
    ...
    >
      ...
      >
        ...
        >
          ...
          <span style={{ color: "var(--text)" }}>
            ₦
            {items.reduce((s, i) => s + ep(i) * i.quantity, 0).toLocaleString()}
          </span>
        </div>
        <div
          style={{
        ...
        >
          ...
          <span
            style={{ color: shipping === 0 ? "var(--accent)" : "var(--text)" }}
          >
      ...
      >
        <span
          style={{ fontWeight: 700, color: "var(--text)", fontSize: "0.9rem" }}
        >
          Total
        </span>
        <span
...
export default function Checkout() {
  ...
  const { user } = useAuthStore(); // ← single call, inside component
  ...
  const subtotalAmt = items.reduce((s, i) => s + ep(i) * i.quantity, 0); // ← uses ep()
  ...
  return (
    ...
    >
      <div
        style={{ maxWidth: 980, margin: "0 auto", padding: "3rem 1.5rem 5rem" }}
      >
        <button
          onClick={() =>
